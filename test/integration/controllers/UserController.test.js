  ...
  describe('/api/user/uri', function () {
    ...
    describe('invalid new user ID:', function () {
      ...
      it('existing user with different case', function (done) {
        uppercaseuri = newUserAttrs.uri.toUpperCase();
        request(sails.hooks.http.app)
          .post('/api/user/uri/'+uppercaseuri)
          .expect(200)
          .expect(resAssert.isTrue)
          .end(done);
      });
