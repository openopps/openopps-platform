...
router.post('/api/v1/application/removeApplicationTask', auth.bearer, async (ctx, next) => {
  var status = await cycleService.checkProcessingStatus(ctx.request.fields.taskId);
  if (status.is_processing) {
    ...
    if (owner)
    {
      var data = await service.removeApplicationTask(ctx.request.fields.taskId, ctx.request.fields.applicationTaskId, ctx.request.fields.taskListApplicationId, ctx.state.user);
      if(!data){
        ctx.status = 401;
