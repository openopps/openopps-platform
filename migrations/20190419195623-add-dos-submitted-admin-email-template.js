...
exports.up = function (db, callback) {
  db.all('SELECT community_id FROM community WHERE community_name = \'U.S. Department of State Student Internship Program (Unpaid)\'', (err, results) => {
    if (results && results[0]) {
      ...
      'VALUES(' + results[0].community_id + ", 'task.update.submitted.admin', 'state.department/internship.update.submitted.admin', 'state.department/layout.html', now());", callback);
    } else {
      callback(err);
    }
  });
