...
exports.up = function (db, callback) {
  return Promise.all([
    db.runSql('drop function get_applicant_score_by_task(bigint)', (err) => {
      db.runSql(`
        ...
        return QUERY
          ...
          from 
            ...
            not exists(
              ...
              and not exists(
                select 1
                from task_list_application_history
                where task_list_application_history.application_id = app.application_id
                  and action = 'delete'
              )
          ) t2
  ...
  `, callback );
    ...
    db.addColumn('task_list_application_history', 'task_id', {  type: 'int', notNull: true, defaultValue: 0 }),
    db.addColumn('task_list_application_history', 'application_id', {  type: 'bigint', notNull: true, defaultValue: 0 }),
  ]);
};
exports.down = function (db) {
  return Promise.all([
    db.removeColumn('task_list_application_history', 'task_id'),
    db.removeColumn('task_list_application_history', 'application_id'),
  ]);
};
